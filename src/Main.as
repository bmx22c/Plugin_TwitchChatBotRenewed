...
// TODO: Fix Strings not sending live but updates when Active is changed
...
[Setting category="General" name="Twitch Username" description="Your Twitch username to confirm that it\'s you that tries to access your data. Ignore if you don\' use the Twitch extension."]
string Setting_TwitchUsername = "";
// Send the Twitch username to the database
...
CTrackManiaNetworkServerInfo@ server_info;
...
void SendInformations(string type, string content, string username, string key)
{
	...
	// req.Url = "https://tm-info.digit-egifts.fr/submit.php";
	// req.Url = "https://tm-info.herokuapp.com/submit?type="+Net::UrlEncode(type)+"&content="+Net::UrlEncode(content)+"&username="+Net::UrlEncode(username)+"&key="+Net::UrlEncode(key);
	req.Url = "http://localhost:3000/submit?type="+Net::UrlEncode(type)+"&content="+Net::UrlEncode(content)+"&username="+Net::UrlEncode(username)+"&key="+Net::UrlEncode(key);
...
void Authenticate()
{
	...
	OpenBrowserURL('https://api.trackmania.com/oauth/authorize?response_type=code&client_id=9a3748e462852e241238&scope=&redirect_uri=http://localhost:3000/redirect&state='+uniqueCode);
}
